...
- 全站使用繁體中文的嚴格標點斷行規則；文章載入時會以瀏覽器的繁體中文詞組分析自動保護詞內斷行，不依固定字數切割，也不需要作者逐一標記一般中文詞。
- 產品名稱、專有名詞、關聯片語或指令組可能橫跨多個分析詞；只有這類不可拆的短語才標記為 `<span class="keep-phrase">不可拆短語</span>`。不要包住完整句子或段落，以免窄螢幕溢出。
...
這個跨詞短語在任何欄寬都不應拆開：<span class="keep-phrase">建立資產索引</span>。
```
