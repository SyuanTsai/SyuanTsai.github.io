...
- 全站使用繁體中文的嚴格標點與語意詞組斷行規則；不依固定字數切割，也不需要作者逐一標記詞組。
- 瀏覽器先依 `word-break: auto-phrase` 自然排版，再由 `Intl.Segmenter` 保護短詞、單字連接詞、中文數字與「不、非、未、可」等詞綴，避免完整語意單位被拆成孤字；短行內程式碼也會自動黏住前一個詞。不支援 `Intl.Segmenter` 時才單獨使用原生規則。
- 一般段落、清單與引用使用受限的閱讀寬度；標題分隔線、程式碼、表格、圖片與提示區塊仍可使用完整內容寬度。
...
瀏覽器會依內容欄寬度與繁體中文詞組規則，自然決定每一行的結束位置。
```
