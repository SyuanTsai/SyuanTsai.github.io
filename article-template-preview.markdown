...
同篇文章有多張圖片時，使用穩定且可辨識的 Asset ID，並建立資產索引。第一筆資產可記在 Issue 本文，後續每張邏輯圖片或同圖尺寸版本各用一則 comment 管理。一般中文詞組由瀏覽器原生規則自然換行，作者不需要加入不可拆標記。
...
| 2026-08-31 | 改用瀏覽器原生詞組斷行，並限制一般內文閱讀寬度 |
{: .update-history}
