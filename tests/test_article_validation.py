...
class ArticleValidationTests(unittest.TestCase):
    ...
    def test_repository_posts_follow_the_complete_article_contract(self) -> None:
        errors, post_count, _ = validate_repository(ROOT)

        self.assertEqual(2, post_count)
        self.assertEqual([], errors)

    def test_taxonomy_requires_one_registered_category_and_one_to_five_tags(self) -> None:
        registered_categories, registered_tags = load_taxonomy_registry(ROOT)
        document = parse_front_matter(
            ROOT / "_posts/2025-12-14-code-review-alignment-for-long-term-maintenance.markdown"
        )
    ...
    def test_parses_inline_lists_and_nested_image(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(["code"], document.fields["categories"])
    ...
    def test_output_url_is_independent_from_taxonomy(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path = self.write_document(
                root,
            ...
            self.assertEqual(
                "/2026/08/29/debugging-timeouts.html",
    ...
    def test_verify_post_compares_generated_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = root / "_site/2026/08/29/generated-metadata.html"
            ...
            output.write_text(
                ...
                <html><head>
                  ...
                  <link rel="canonical" href="https://example.com/2026/08/29/generated-metadata.html">
                </head><body></body></html>
                """,
                encoding="utf-8",
            )
